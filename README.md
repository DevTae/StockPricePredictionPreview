...
#### 현재 진행 상황
  ...
  - 위와 같이 `Disk I/O` 를 줄였음에도 학습 진행 중 똑같이 `Deadlock` 현상이 발생하였고, 추가적으로 `Memory Leak` 현상이 의심되어 `torch`, `cpu` 변수를 할당 해제하였다.
  - 앞으로는 하이퍼파라미터 튜닝을 진행하며 라이브러리인 `optuna` 또한 학습해볼 예정이다.
  - 현재 출력층에서 부정부터 긍정까지 `0 ~ 100` 사이에서 표현하도록 하였고, `평균 오차 목표`를 `10%` 정도로 잡았고, 이에 따라 `PMSE` 가 `25` 정도가 되는 것이 목표이다.
