...
#### 현재 진행 상황
  ...
  - 학습 중 Loss 가 줄어들지 않고 있는 상황임을 발견하였다.
    ...
    - Scaling 기준 변경 (입력값은 `-1 ~ 1` 로 스케일링하였고, 출력값은 `0 ~ 1` 사이로 스케일링 진행)
      - 따라서, `목표 MSE` 는 출력층 데이터에서의 `0.1 정도 오차`로 `0.01` 로 재설정
  - 위 시도를 통하여, Loss 가 수렴하는 것을 확인할 수 있었다.
...
#### 현재 진행 상황
  - 이전에 학습 중 멈췄던 것을 `Deadlock` 이라고 판단하였었지만 추가적인 원인으로 `CPU 할당률`이 100% 가 넘는 것을 의심하였고 설정 변경을 시도하였다.
...
#### 현재 진행 상황
  ...
  - 캐시 적용 유무에 따라 메모리 변동이 다르기에 `캐시를 적용한 것`과 `적용하지 않은 것` 사이에서 비교(`psutil` 라이브러리 활용)를 진행해보았다.
  ...
  - 그에 따라, 메모리 동향을 `guppy3` 라이브러리를 통하여 확인해보았다.
