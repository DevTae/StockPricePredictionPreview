...
- 모델 학습 및 하이퍼파라미터 튜닝
  - `LSTM` -> `Dropout` -> `LayerNorm` -> `FC` -> `Dropout` -> `BatchNorm` -> `FC` 으로 모델 구성
...
#### 진행사항
  ...
  - `주가 분석 모델 툴킷 제작` 및 `모델 설계` (LSTM 기반) 진행
  - `하이퍼 파라미터 설정` 및 학습 진행
...
#### 하이퍼파라미터 튜닝 및 전처리 스케일링 방식 변경
  ...
  - 학습률, Epoch, LSTM 레이어 개수, Hidden Dimension 에 대한 하이퍼파라미터 튜닝을 진행하였다.
...
#### 현재 학습 결과
  ...
  - 이전의 CNN + RNN 결합한 모델과 그렇게 큰 차이를 보이진 않는 것으로 보여, `LSTM 모델`을 바탕으로 진행하고자 한다.
...
#### 현재 진행 상황
  - 모델 구조 개선 완료
    - `LSTM` -> `Dropout` -> `LayerNorm` -> `FC` -> `Dropout` -> `BatchNorm` -> `FC` 으로 모델을 구성하였다.
